...
$GLOBALS['WKPDF_BASE_SITE']='https://'.$_SERVER['SERVER_NAME'].'/';
...
class WKPDF {
    ...
    public function __construct(){
        ...
        do{
            $this->tmp=FCPATH.'tmp/'.mt_rand().'.html';
    ...
    public function output($mode,$file){
        switch($mode){
            ...
            case self::$PDF_SAVEFILE:
                return file_put_contents(FCPATH.'tmp/'.$file,$this->pdf);
                break;
            default:
        ...
        // echo $this->tmp; 
        // Delete the file found in $this->tmp folder
        $this->pdf=self::_pipeExec('rm -f "'.$this->tmp.'"');
        return false;
    }
}// END